	...
	var SVGMap = (function () {
		...
		SVGMap.prototype.render = function () {
			...
			for (var state in mapConfig.shapes) {
				var thisStateData = mapConfig['names'][state],
					initColor = '#' + (thisStateData && thisStateData.color || opt.stateInitColor),
				...
				var x = obj.getBBox().x + mapConfig['names'][state].x,
					y = obj.getBBox().y + mapConfig['names'][state].y;
				//默认显示文本
				r.text(x, y, obj.name).attr({
					cursor: "pointer",
					metadata: {
						mapId: 1
					}
